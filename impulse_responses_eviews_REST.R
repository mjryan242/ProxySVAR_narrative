...
Eviews <- read.csv(file = 'eviews_IR1510.csv')
...
ir<- function(i,name) {
ImpulseR<-as.data.frame(cbind(SW[1:20 ,i],SW[1:20 ,i+6],SW[1:20 ,i+12],(Eviews[1:20 ,i]/0.50),Periods))

var<-paste0(name)
colnames(ImpulseR)<-c("Proxy","L","H","Recusive","Periods" )
...
chart<-
    ...
    #  geom_ribbon(aes(ymin = L, 
      #ymax=H), colour="lightblue", fill="blue",alpha=0.1)+
       geom_hline(yintercept=0)+ggtitle(paste0(var,"\n"))+
 geom_line(aes(y = Recusive), color = "black", linetype = "dotted") +
...
library(gridExtra)
IR_graphs_eviews<-grid.arrange(chartUnc, chartSha, chartOut,  chartInf,chartNom, chartRER, nrow = 3)

ggsave(filename="IR_graphs_eviews_REST.pdf", plot=IR_graphs_eviews, width = 10, height = 10,device=cairo_pdf, units = "cm")
