...
application = 'Uncertainty';  % Name of this empirical application. This name will be used for creating and accessing folders

p           = 4;     %Number of lags in the VAR model
 
confidence  = .9;    %Confidence Level for the standard and weak-IV robust confidence set

% Define the variables in the SVAR
columnnames = [{'Uncertainty'}, ...
               {'Stock prices'}, ...
               {'Ygap'}, ...
               {'Inflation'}, ...
               {'IR'}, ...
...
time        = 'quarterly';  % Time unit for the dataset (e.g. year, month, etc).
...
cd(strcat(pwd,'/Data/',application));
    ...
    ydata = xlsread('data_for_SVARIV'); 
 
%for my narrative instrument
  %  z    = xlsread('Narrative_IV_m.xlsx');
 %for bloom instrument  
   z    = xlsread('ExtIV_bloom.xlsx');  
    years = xlsread('timeU');
    
dataset_name = 'UData'; %The name of the dataset used for generating the figures (used in the output label)
...
NB = 10000;                      % Number of bootstrap replications 
